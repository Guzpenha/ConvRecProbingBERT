...
class EvalHooks(tf.train.SessionRunHook):
    ...
    def begin(self):
        ...
        self.custom_list = {}
        self.custom_list_ids = {}
        self.all_preds = []
        if FLAGS.use_custom_lists:
            df = pd.read_csv(FLAGS.dataset_list_valid)
            for idx, r in df.iterrows():
                k = ",".join(r['seq'].split(",")[0:-1])+"]"
                self.custom_list[k] = [int(v) for v in
                                             r['test_items'][1:-1].split(",")]
                self.custom_list_ids[k] = idx
    ...
    def end(self, session):
        print(
            ...
            format(self.ndcg_1 / self.valid_user, self.hit_1 / self.valid_user,
                   ...
                   self.valid_user))
        df = pd.DataFrame(self.all_preds, columns=['order']+['prediction_'+str(i)
                                            for i in range(len(self.all_preds[0])-1)])
        if not os.path.isdir(FLAGS.output_predictions_folder):
            os.makedirs(FLAGS.output_predictions_folder)

        df.sort_values(by='order', ascending=True)\
            [['prediction_'+str(i) for i in range(len(self.all_preds[0])-1)]] \
            .to_csv(FLAGS.output_predictions_folder+"/predictions.csv", index=False)
...
#               masked_lm_ids.shape, info.shape)
        ...
        for idx in range(len(input_ids)):
            ...
            if FLAGS.use_custom_lists:
              ...
              rated_ids = self.user_history["user_" + str(info[idx][0])][0]
              seq_as_key = "["+", ".join(
                  [vocab.id_to_tokens[r].split("item_")[1] for r in rated_ids])+"]"
              item_idx = []
              unknown_items_idx = []
              for i, item_token in enumerate(self.custom_list[seq_as_key]):
                  if "item_"+str(item_token) not in vocab.token_to_ids:
...
def main(_):
    ...
    if FLAGS.do_eval:
        ...
        with open(FLAGS.output_predictions_folder+'/config.json', "w") as f:
            args = bert_config.to_dict()
            args["task"] = FLAGS.dataset_list_valid.split("_")[1].replace(".csv", "")
            args["seed"] = str(FLAGS.seed)
            args["recommender"] = "BERT4Rec"
            parameters = {
                "args": args
            }

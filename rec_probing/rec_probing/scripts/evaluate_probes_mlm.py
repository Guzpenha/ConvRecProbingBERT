...
def main():
    ...
    year_agg = dfs_raw[dfs_raw["model"] == "bert-large-cased"].\
            ...
            groupby(["probe", "sentence_type", "task", "model", "year"])[["R@10", "R@50"]].\
            agg(["mean", "count"])    
    year_agg = year_agg.reset_index()
    year_agg.columns = ["probe", "sentence_type", "task", "model", "year", "R@10", "count", "R@50", "count_1"]
    ...
    dfs_raw_corr = dfs_raw_corr.merge(df_wiki_all[["task","item","in_wiki"]],
    ...
    for sentence_type in ['type-I', 'type-II']:
        for task in tasks:
            ...
            cor_and_pvalues = []
            for wiki_col in ["item_in_wiki"]:
                cor, pvalue = scipy.stats.pearsonr(df_filtered["R@10"], df_filtered[wiki_col])
                cor_and_pvalues.append(cor)
                cor_and_pvalues.append(pvalue)
            for pop_col in ["pop", "log_pop"]:
                cor, pvalue = scipy.stats.pearsonr(df_filtered["R@10"], df_filtered[pop_col])
                cor_and_pvalues.append(cor)
                cor_and_pvalues.append(pvalue)
            for bert_score_col in ["BERT_score_avg_5", "BERT_score_avg_10"]:
                cor, pvalue = scipy.stats.pearsonr(df_filtered["R@10"], df_filtered[bert_score_col])
                cor_and_pvalues.append(cor)
                cor_and_pvalues.append(pvalue)
            cor, pvalue = scipy.stats.pearsonr(df_filtered["R@10"], df_filtered["title_std_token_percentage"])
            cor_and_pvalues.append(cor)
            cor_and_pvalues.append(pvalue)
            cor, pvalue = scipy.stats.pearsonr(df_filtered["R@10"], df_filtered["word_movers_distance"])
            cor_and_pvalues.append(cor)
            cor_and_pvalues.append(pvalue)

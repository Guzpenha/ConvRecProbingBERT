...
def recall_at_with_cand(preds, labels, candidates, at=1):
    ...
    preds, labels  = zip(*sorted(zip(preds, labels),reverse=True))
...
def main():
    ...
    df_tests.to_csv(args.output_folder+"statistical_tests_probe_results.csv", sep="\t")
    ...
    agg_df[final_order].\
        to_csv(args.output_folder+"aggregate_probe_results.csv", sep="\t")

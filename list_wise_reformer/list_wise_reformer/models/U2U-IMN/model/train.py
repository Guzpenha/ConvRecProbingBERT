...
tf.flags.DEFINE_string("task", "", "dataset name")
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        grads_and_vars = optimizer.compute_gradients(u2u_imn.mean_loss)
        ...
        """
        loss_summary = tf.scalar_summary("loss", u2u_imn.mean_loss)
        acc_summary = tf.scalar_summary("accuracy", u2u_imn.accuracy)
        ...
        def train_step(x_utterances, x_response, x_utterances_len, x_response_len, 
            ...
            feed_dict = {
              ...
              u2u_imn.dropout_keep_prob: FLAGS.dropout_keep_prob,
            ...
            _, step, loss, accuracy, predicted_prob = sess.run(
                [train_op, global_step, u2u_imn.mean_loss, u2u_imn.accuracy, u2u_imn.probs],
                feed_dict)
        ...
        def dev_step():
            ...
            num_test = 0
            num_correct = 0.0
            valid_batches = data_helpers.batch_iter(valid_dataset, FLAGS.batch_size, 1, target_loss_weight, FLAGS.max_utter_len, FLAGS.max_utter_num, FLAGS.max_response_len, charVocab, FLAGS.max_word_length, shuffle=False)
            for valid_batch in valid_batches:
                ...
                batch_accuracy, predicted_prob = sess.run([u2u_imn.accuracy, u2u_imn.probs], feed_dict)
            ...
            with open(os.path.join(FLAGS.output_predictions_folder, 'config.json'), 'w') as f:
                conf = {}
            ...
            df.to_csv(FLAGS.output_predictions_folder + "/predictions.csv", index=False)
        ...
        for batch in tqdm(batches):
            x_utterances, x_response, x_utterances_len, x_response_len, x_utters_num, x_responses_num, x_dist, x_target, x_target_weight, id_pairs, x_u_char, x_u_char_len, x_r_char, x_r_char_len = batch
            train_step(x_utterances, x_response, x_utterances_len, x_response_len, x_utters_num, x_responses_num, x_dist, x_target, x_target_weight, id_pairs, x_u_char, x_u_char_len, x_r_char, x_r_char_len)
